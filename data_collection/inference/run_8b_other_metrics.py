...
Batched two-stage Qwen3-VL-8B-Thinking analyzer.
...
def load_intermediates_from_output_dir(dir_path: Path) -> List[IntermediateResult]:
    ...
    for json_file in json_files:
        ...
        # Normalise image paths: support both single and multi-image formats.
        # After normalisation, ExampleInput always has resolved_image_paths set.
...
class QwenVLThinkingAnalyzer:
    ...
    def _build_teacher_forced_tensors(
        self,
        inputs: Dict[str, Any],
        generated_token_ids_batch: List[List[int]],
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, int, int]:
        """
        Append generated token IDs to the prompt tensors.

        Returns:
            full_input_ids, full_attention_mask, gen_attn,
            prompt_block_len, max_gen_len
        """
        ...
        return full_input_ids, full_attention_mask, gen_attn, prompt_block_len, max_gen_len
    ...
    def compute_token_metrics_from_inputs(
        ...
        (
            img_full_ids, img_full_attn, gen_attn,
            img_prompt_block_len, max_gen_len,
        ) = self._build_teacher_forced_tensors(image_inputs, generated_token_ids_batch)
        ...
        (
            txt_full_ids, txt_full_attn, _,
            txt_prompt_block_len, _,
        ) = self._build_teacher_forced_tensors(text_inputs, generated_token_ids_batch)
        ...
        image_outputs = self.model(
            input_ids=img_full_ids,
            attention_mask=img_full_attn,
        ...
        text_outputs = self.model(
            input_ids=txt_full_ids,
            attention_mask=txt_full_attn,
            output_hidden_states=False,
            return_dict=True,
            use_cache=False,
            **txt_static,
        ...
        for start in range(0, max_gen_len, analysis_chunk_size):
            ...
            # The token IDs that were actually selected at each of these positions.
            # gen_ids_tensor indices: chunk columns correspond to generated positions start..end
            selected_ids = img_full_ids[:, img_prompt_block_len + start : img_prompt_block_len + end]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Batched Qwen3-VL-8B-Thinking: generation + per-token metrics analysis"
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "--prefill-output-dir spatial_mm_qwen3vl_8b_outputs "
            "--prefill-output-dir erqa_qwen3vl_8b_outputs"
        ),
    )
...
def main(cli_args=None) -> None:
    ...
    if args.prefill_output_dir:
        # ------------------------------------------------------------------
        # Prefill mode: load pre-generated outputs, skip Pass 1 entirely.
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # Normal mode: Pass 1 — generate responses from scratch.
        # ------------------------------------------------------------------
