...
Batched three-stage Qwen3-VL-8B-Thinking analyzer with multi-image support.
...
5. Pass 3: batched teacher-forced forward pass with NO image
   ...
   - computes per-token JS divergence between:
       final(with image[s]) vs final(no image)
...
A) Batched dataset via JSONL:
   each line can be one JSON object like:
   {"id": "ex1", "image_path": "/path/to/img1.jpg", "prompt": "Describe this image."}
   {"id": "ex2", "image_url": "https://...", "prompt": "What is happening here?"}
   {"id": "ex3", "image_paths": ["/path/to/img1.jpg", "/path/to/img2.jpg"], "prompt": "Compare the images."}
   {"id": "ex4", "image_urls": ["https://...", "https://..."], "prompt": "Compare the images."}

B) Single example via CLI:
   --image-path ... --prompt ...
   --image-url ... --prompt ...
   --image-paths img1 img2 img3 ... --prompt ...
   --image-urls url1 url2 url3 ... --prompt ...
...
class ExampleInput:
    ...
    # resolved local file paths actually given to the model
...
def normalize_optional_str_list(x: Optional[Any]) -> Optional[List[str]]:
    if x is None:
        return None
    if isinstance(x, list):
        out = [str(v) for v in x if str(v).strip()]
        return out if out else None
    if isinstance(x, str):
        s = x.strip()
        return [s] if s else None
    raise TypeError(f"Expected None, str, or list[str], got {type(x)}")


def validate_example_image_fields(example: ExampleInput) -> None:
    single_count = int(example.image_path is not None) + int(example.image_url is not None)
    multi_count = int(example.image_paths is not None) + int(example.image_urls is not None)
    ...
    if multi_count > 1:
        raise ValueError(
            f"Example {example.example_id}: provide only one of image_paths or image_urls."
        )

    if single_count > 0 and multi_count > 0:
        raise ValueError(
            f"Example {example.example_id}: do not mix singular and plural image fields."
        )


def resolve_example_images(example: ExampleInput, download_dir: Path) -> ExampleInput:
    validate_example_image_fields(example)

    resolved: List[str] = []

    # multi local paths
    if example.image_paths is not None:
        for p_str in example.image_paths:
            p = Path(p_str)
            if not p.exists():
    ...
    if example.image_urls is not None:
        for i, url in enumerate(example.image_urls):
            filename = (
                f"{sanitize_filename(example.example_id)}_{i:03d}_"
    ...
    if example.image_path is not None:
        ...
        example.resolved_image_paths = [str(p)]
        return example
    ...
    raise ValueError(
        f"Example {example.example_id}: provide image_path, image_url, image_paths, or image_urls."
    )
...
def load_examples_from_jsonl(jsonl_path: Path) -> List[ExampleInput]:
    ...
    with open(jsonl_path, "r", encoding="utf-8") as f:
        for idx, line in enumerate(f, start=1):
            ...
            image_paths = normalize_optional_str_list(obj.get("image_paths"))
            image_urls = normalize_optional_str_list(obj.get("image_urls"))
...
def load_examples_from_args(args) -> List[ExampleInput]:
    ...
    num_modes = sum(
        [
            int(args.image_path is not None),
            int(args.image_url is not None),
            int(bool(args.image_paths)),
            int(bool(args.image_urls)),
        ]
    )

    if num_modes == 0:
        raise ValueError(
            "For single-example mode, provide one of --image-path, --image-url, "
            "--image-paths, or --image-urls."
        )
    if num_modes > 1:
        raise ValueError(
            "For single-example mode, provide only one of --image-path, --image-url, "
            "--image-paths, or --image-urls."
        )
    ...
    if args.image_path:
        ...
        example_id = sanitize_filename(Path(args.image_paths[0]).stem)
...
class QwenVLThinkingAnalyzer:
    ...
    def build_messages(
        ...
        if include_image:
            if not image_paths:
                raise ValueError("include_image=True but image_paths is missing.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # single-image backward-compatible CLI
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="One or more local image paths for a single example."
    )
    parser.add_argument(
        "--image-urls",
        type=str,
        nargs="+",
        default=None,
        help="One or more image URLs for a single example."
    )
...
def main(cli_args=None) -> None:
    ...
    for ex in examples:
        resolved_examples.append(resolve_example_images(ex, download_dir=download_dir))
