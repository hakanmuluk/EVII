...
METRIC_NAME = "avg_noimage_js_until_bocpd_beta_stop"
...
def _beta_pdf(x, alpha, beta, eps=1e-12):
    ...
    # avoid underflow to exact zero too aggressively
    if log_pdf < -745.0:
        return 0.0
...
def _bocpd_run_length_posterior_beta(
    ...
    This is a practical beta-family predictive model for bounded signals in (0, 1).
    """
    x = np.asarray(x, dtype=float)
    T = len(x)
    ...
    # Sufficient stats per candidate previous run length:
    # count_r and sum_x_r for the current regime under that run-length hypothesis
    ...
    for t in range(1, T + 1):
        ...
        prev_rl_count = t  # previous run lengths 0..t-1
        ...
        # Update sufficient stats after observing xt
        new_count_params = np.empty(t + 1, dtype=float)
        new_sum_params = np.empty(t + 1, dtype=float)

        # New regime after changepoint resets to prior state
...
def _choose_k_bocpd_beta(
    ...
    Uses your requested dynamic rules:
    ...
    Accept the earliest t in [k_min, search_end] such that:
      1) changepoint probability at t is high enough
    ...
    #search_end = min(total_tokens, int(k_max))
    if search_end <= 0:
        return 0, {
            "k_min": int(k_min),
            "k_max": int(k_max),
...
def compute_metric_for_example(record, offset, **params):
    ...
    chosen_k is selected examplewise by beta-family BOCPD from the aligned
    ...
    # Remaining stop-rule parameters
    ...
    cp_prob_threshold = float(params.get("adaptive_k_cp_prob_threshold", 0.20))
    pre_window = int(params.get("adaptive_k_pre_window", 5))
    post_window = int(params.get("adaptive_k_post_window", 4))
    drop_ratio = float(params.get("adaptive_k_drop_ratio", 0.65))
