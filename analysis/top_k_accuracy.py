...
Results are printed as a table and saved to <output-dir>/top_k_accuracy.{json,csv}.

Usage:
    python top_k_accuracy.py \\
        --json-dirs               d1,d2,...,d6 \\
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Top-K accuracy analysis: for each version, rank examples by the "
            "dynamic BOCPD metric and report accuracy at the top 10/20/30%."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Output folder for top_k_accuracy.json and top_k_accuracy.csv",
    )
    parser.add_argument(
    ...
    parser.add_argument("--adaptive-k-cp-prob-threshold",  type=float, default=0.20)
    ...
    parser.add_argument("--adaptive-k-drop-ratio",         type=float, default=0.65)
    ...
    print("=" * 80)
    print(f"Top-K Accuracy Analysis  —  {dynamic_metric.METRIC_NAME}")
    ...
    for vname, jdir, rjson, odir in zip(version_names, json_dirs, results_jsons, other_dirs):
        ...
        res = compute_top_k_accuracy(records, bocpd_params, percentiles=percentiles)
    ...
    # --- Save outputs ---
    json_path = output_dir / "top_k_accuracy.json"
    csv_path  = output_dir / "top_k_accuracy.csv"
