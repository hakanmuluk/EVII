#!/usr/bin/env python3
"""
Top-K Accuracy Analysis (inference metric)

For each version (model × dataset combination), computes the metric defined in
inference_metric_definition_dynamic.py for every example, then reports:

  - Overall accuracy across all valid examples
  - Accuracy among the top 10%, 20%, and 30% highest-scoring examples

Results are printed as a table and saved to <output-dir>/top_k_accuracy_inference.{json,csv}.

Usage:
    python top_k_accuracy_inference.py \\
        --json-dirs               d1,d2,...,d6 \\
        ...
        --output-dir              top_k_inference_run \\
        --offset                  0 \\
...
import inference_metric_definition_dynamic as dynamic_metric
...
def compute_top_k_accuracy(
    ...
    """
    Compute metric for each record, sort descending, then compute accuracy
    for the overall set and each top-k percentile slice.

    Returns a dict with:
        n_total      – number of loaded records
        n_valid      – records with a finite metric value
        overall_acc  – accuracy over all valid records
        top_k        – dict keyed by percentile string (e.g. "0.10") with
                       keys: n, correct, accuracy
        per_example  – list of {example_id, metric_value, correct} sorted desc
    """
...
def print_results_table(
    ...
    pct_headers = [f"top{int(p * 100)}%" for p in percentiles]
    ...
    for vname, res in version_results:
        row_parts = (
            ...
            + [_fmt_acc(res["top_k"][k]["accuracy"]).rjust(col_w) for k in pct_keys]
            + [str(res["top_k"][k]["n"]).rjust(8) for k in pct_keys]
        )
...
def print_avg_delta_table(
    version_results: List[Tuple[str, dict]],
    percentiles: List[float],
) -> None:
    """Print average (topX% acc - overall_acc) across all versions for each percentile."""
    pct_keys    = [f"{p:.2f}" for p in percentiles]
    pct_headers = [f"top{int(p * 100)}%" for p in percentiles]

    col_w = 14
    n_versions = len(version_results)

    print(f"  Average lift (topX% acc − overall_acc) across {n_versions} version(s):")

    deltas: dict = {k: [] for k in pct_keys}
    for _, res in version_results:
        oa = res["overall_acc"]
        if not math.isfinite(oa):
            continue
        for k in pct_keys:
            acc = res["top_k"][k]["accuracy"]
            if math.isfinite(acc):
                deltas[k].append(acc - oa)

    parts = []
    for k, hdr in zip(pct_keys, pct_headers):
        vals = deltas[k]
        if vals:
            mean_d = sum(vals) / len(vals)
            sign = "+" if mean_d >= 0 else ""
            parts.append(f"  {hdr}: {sign}{mean_d * 100:.2f}%  (n={len(vals)})")
        else:
            parts.append(f"  {hdr}: N/A")
...
def save_csv(
    ...
    fieldnames = (
        ...
        + [f"top{int(float(k)*100)}_n" for k in pct_keys]
        + [f"top{int(float(k)*100)}_correct" for k in pct_keys]
        + [f"top{int(float(k)*100)}_acc" for k in pct_keys]
    )
    ...
    for vname, res in version_results:
        ...
        for k in pct_keys:
            ...
            tk = res["top_k"][k]
            row[f"top{pct_int}_n"]       = tk["n"]
            row[f"top{pct_int}_correct"]  = tk["correct"]
            acc = tk["accuracy"]
            row[f"top{pct_int}_acc"] = acc if math.isfinite(acc) else ""
        rows.append(row)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Top-K accuracy analysis (inference metric): for each version, rank examples by the "
            "inference metric and report accuracy at the top 10/20/30%."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Output folder for top_k_accuracy_inference.json and top_k_accuracy_inference.csv",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Comma-separated top-k percentiles to evaluate (default: 0.10,0.20,0.30)",
    )

    # Inference metric hyperparameters
    ...
    print("=" * 80)
    print(f"Top-K Accuracy Analysis (inference metric)  —  {dynamic_metric.METRIC_NAME}")
    ...
    for vname, jdir, rjson, odir in zip(version_names, json_dirs, results_jsons, other_dirs):
        ...
        res = compute_top_k_accuracy(records, metric_params, percentiles=percentiles)
        ...
        print(
            ...
            + "  ".join(
                f"top{int(float(k)*100)}%={_fmt_acc(res['top_k'][k]['accuracy'])} (n={res['top_k'][k]['n']})"
                for k in pct_keys
            )
        )
    ...
    print_avg_delta_table(version_results, percentiles)

    # --- Save outputs ---
    json_path = output_dir / "top_k_accuracy_inference.json"
    csv_path  = output_dir / "top_k_accuracy_inference.csv"

    save_json(version_results, metric_params, json_path)
