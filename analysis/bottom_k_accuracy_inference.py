#!/usr/bin/env python3
"""
Bottom-K Accuracy Analysis (inference metric)
...
Results are printed as a table and saved to <output-dir>/bottom_k_accuracy_inference.{json,csv}.

Usage:
    python bottom_k_accuracy_inference.py \\
        --json-dirs               d1,d2,...,d6 \\
        ...
        --output-dir              bottom_k_inference_run \\
        --offset                  0 \\
...
def print_avg_delta_table(
    ...
    """Print average (botX% acc - overall_acc) across all versions for each percentile."""
    pct_keys    = [f"{p:.2f}" for p in percentiles]
    pct_headers = [f"bot{int(p * 100)}%" for p in percentiles]

    n_versions = len(version_results)

    print(f"  Average lift (botX% acc − overall_acc) across {n_versions} version(s):")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Bottom-K accuracy analysis (inference metric): for each version, rank examples by the "
            "inference metric and report accuracy at the bottom 10/20/30%."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Output folder for bottom_k_accuracy_inference.json and bottom_k_accuracy_inference.csv",
    )
    parser.add_argument(
    ...
    print("=" * 80)
    print(f"Bottom-K Accuracy Analysis (inference metric)  —  {dynamic_metric.METRIC_NAME}")
    ...
    for vname, jdir, rjson, odir in zip(version_names, json_dirs, results_jsons, other_dirs):
        ...
        res = compute_bottom_k_accuracy(records, metric_params, percentiles=percentiles)
    ...
    # --- Save outputs ---
    json_path = output_dir / "bottom_k_accuracy_inference.json"
    csv_path  = output_dir / "bottom_k_accuracy_inference.csv"
