#!/usr/bin/env python3
"""
Top-K accuracy analysis for simple token-level metrics, computed over the
...
Metrics evaluated (over first-k tokens):
  avg_log_prob         higher = more confident
  avg_neg_entropy      higher = lower entropy = more confident
  avg_neg_perplexity   higher = lower perplexity = more confident
  avg_self_certainty   higher = more certain

For each (k_spec, metric, version) combination the script ranks examples by the
first-k metric value (descending) and reports accuracy at the overall level and
at the top 10/20/30% thresholds.

Results are printed as tables and saved to:
  <output-dir>/top_k_accuracy_firstk.json
  <output-dir>/top_k_accuracy_firstk.csv

Usage:
    python top_k_accuracy_other_metrics_firstk.py \\
        --k-specs "bocpd,constant:10,constant:30,constant:50,ratio:0.05,ratio:0.10,ratio:0.15" \\
        --percentiles "0.10,0.20,0.30" \\
        --output-dir top_k_firstk_run
"""
...
# ---------------------------------------------------------------------------
# Top-k accuracy computation  (sort descending — highest scores first)
# ---------------------------------------------------------------------------
...
def compute_top_k_accuracy(
    ...
    finite values, sort descending and compute top-k accuracy at each percentile.
    """
    n_valid = len(scored)
    ...
    scored = sorted(scored, key=lambda x: x[0], reverse=True)   # highest first
    ...
    for p in percentiles:
        ...
        top_k[f"{p:.2f}"] = {"n": k, "correct": nc, "accuracy": nc / k}
...
def print_table(
    ...
    pct_hdrs = [f"top{int(p * 100)}%" for p in percentiles]
    ...
    for vname, res in version_results:
        ...
        deltas = []
        for k in pct_keys:
            top_acc = res["top_k"][k]["accuracy"]
            if math.isfinite(overall) and math.isfinite(top_acc):
                d = top_acc - overall
        ...
        row_parts = (
            ...
            + [_fmt(res["top_k"][k]["accuracy"]).rjust(col_w) for k in pct_keys]
        ...
        for metric_name, per_version in per_metric.items():
            for vname in case_names:
                ...
                for k, pi in zip(pct_keys, pct_int_keys):
                    tk = res.get("top_k", {}).get(k, {})
                    row[f"top{pi}_n"]       = tk.get("n", "")
                    row[f"top{pi}_correct"]  = tk.get("correct", "")
                    acc = tk.get("accuracy", float("nan"))
                    row[f"top{pi}_acc"]     = acc if (tk and math.isfinite(acc)) else ""
                rows.append(row)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Top-K accuracy analysis for token-level metrics computed over "
    ...
    parser.add_argument(
        ...
        help="Vision offset for BOCPD k-selection (default: 0).",
    )
    parser.add_argument(
        "--percentiles", type=str, default="0.10,0.20,0.30",
        help="Comma-separated top-k percentiles (default: 0.10,0.20,0.30).",
    )
    parser.add_argument(
        "--output-dir", type=str, default="top_k_firstk_run",
        help="Output folder (default: top_k_firstk_run).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Comma-separated subset of metrics (default: all four).",
    )
    ...
    print("=" * 80)
    print("Top-K Accuracy — first-k token metrics")
    ...
    for k_spec in k_specs:
        ...
        for metric_name in metrics_to_run:
            ...
            for cname, records in case_records:
                ...
                n_total = len(records)
                res = compute_top_k_accuracy(scored, percentiles)
                res["n_total"] = n_total
    ...
    # Save outputs
    # ------------------------------------------------------------------
    json_path = output_dir / "top_k_accuracy_firstk.json"
    csv_path  = output_dir / "top_k_accuracy_firstk.csv"
